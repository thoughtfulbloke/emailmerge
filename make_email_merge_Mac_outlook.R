...
# if you want to automatically send, change open theMessage to send theMessage
...
set theAttachmentFile to "@theUnixPath@" as POSIX file as alias
set theMessage to make new outgoing message with properties {subject:theSubject}
make new recipient at theMessage with properties {email address:{address:theAddress}}
tell theMessage
make new attachment with properties {file:theAttachmentFile}
end tell
set plain text content of theMessage to theContent
open theMessage
end tell
'
